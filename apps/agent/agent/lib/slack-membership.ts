...
function explain(error: string): string {
	switch (error) {
		case "no_user_grant":
			return "This workspace did not grant Nexraft permission to add itself to a private channel.";
		case "channel_not_found":
			return "Slack cannot see this channel. A member has to invite Nexraft.";
		case "is_archived":
			return "This channel is archived. Somebody has to unarchive it before Nexraft can join.";
		case "missing_scope":
		...
		case "unknown_bot_user":
			return "Slack did not report which user Nexraft is.";
		default:
...
export async function createSlackChannel(
	...
	if (!token) {
		return {
			error: isPrivate
				? "This workspace did not grant Nexraft permission to create a private channel."
