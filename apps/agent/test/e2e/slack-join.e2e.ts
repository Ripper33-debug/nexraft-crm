...
async function main() {
	...
	if (!JOINS_FOR_REAL) {
		console.log(
			"NOTE  Nexraft joins no channel in this run. A join is permanent.",
		);
	}
	...
	if (!publicUnjoined) {
		record(
			...
			true,
			"skipped: Nexraft is already in every public channel",
		);
	...
	if (!privateUnjoined) {
		record(
			...
			true,
			"skipped: no private channel is visible to Nexraft",
		);
	} else if (!JOINS_FOR_REAL) {
	...
	record(
		...
		canInvite,
		canInvite
			? "a user token lets Nexraft add itself to a private channel"
			: "no user token, so Nexraft cannot add itself to a private channel",
	);
