...
function describe(channel: PickerChannel, canInviteItself: boolean): string {
	...
	if (channel.isMember) return `Nexraft is in${people}`;
	if (!channel.classified) return `Not read from Slack yet${people}`;
	if (!channel.isPrivate) return `Nexraft can join this one${people}`;
	if (canInviteItself) return `Private. Nexraft joins as you${people}`;
	if (channel.inviteRequestedAt)
		return `Private. Waiting on an invite${people}`;
	return `Private. Someone inside has to invite Nexraft${people}`;
}
