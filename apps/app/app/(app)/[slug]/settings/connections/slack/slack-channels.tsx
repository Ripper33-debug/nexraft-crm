...
const INVITE_COMMAND = "/invite @Nexraft";
			...
			onSuccess: async (result) => {
				...
				toast.success(
					result.alreadyJoined
						? "Nexraft is already in there."
						: result.queued
							? "Nexraft is joining."
							: "Ask someone inside to invite Nexraft.",
				);
			},
	...
	return (
		<section className="flex flex-col gap-3 px-(--spacing-block-inline)">
			<div className="flex items-end justify-between gap-4">
				<div>
					<h2 className="font-medium text-sm">Channels Nexraft can reach</h2>
			...
			{channels.stalled ? (
				<p className="text-warning text-xs">
					Nexraft is not reading Slack right now. The list can be out of date.
				</p>
			) : null}
					...
					<p className="px-4 py-4 text-muted-foreground text-sm">
						{channels.pending
							...
							: query
								...
								: "No channels yet. Nexraft reads the list from Slack after it connects."}
					</p>
				}
		...
		<AlertDialog open onOpenChange={(open) => !open && onCancel()}>
			<AlertDialogContent>
				<AlertDialogHeader>
					<AlertDialogTitle>
						{canInviteItself
							? `Add Nexraft to #${channel.name}?`
							: "Ask someone to add Nexraft"}
					</AlertDialogTitle>
					<AlertDialogDescription>
						{canInviteItself
							? `It is a private channel, so Nexraft joins as you. Same as typing the invite yourself. Everyone in the channel sees it join. It reads nothing until you turn a permission on.`
							: `We cannot add Nexraft to a private channel yet. Someone already in #${channel.name} has to run this.`}
					</AlertDialogDescription>
				</AlertDialogHeader>
				...
				<AlertDialogFooter>
					...
					>
						<AsyncButtonContent pendingLabel="Adding…" status={status}>
							{canInviteItself ? "Add Nexraft" : "Copy and mark as asked"}
						</AsyncButtonContent>
					</Button>
				</AlertDialogFooter>
			</AlertDialogContent>
